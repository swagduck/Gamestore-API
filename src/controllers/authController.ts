...
const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: savedUser._id as any, email: savedUser.email, isAdmin: savedUser.isAdmin },
...
const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: user._id as any, email: user.email, isAdmin: user.isAdmin },
      (process.env.JWT_SECRET as string),
      { expiresIn: "7d" }
    );
...
export default { register, login, googleLogin, forgotPassword, resetPassword, logout, getMe, refreshToken };
